...
int main(int argc, char* argv[]) {
	...
	int** G1; // Ma tran trong so cua do thi
	int** D1 = new int* [n]; // Ma tran chua cac gia tri khoang cach ngan nhat tu i => j
	int** S1 = new int* [n]; // Ma tran chua gia tri phan tu ngay sau cua i tren duong di ngan nhat tu i => j
	NhapDoThi(n, G1, 1);
	Floyd(n, G1, D1, S1);
		...
		if (SDL_PollEvent(&g_event) != 0 /*|| load == 1*/) {
			...
			if (x != -1 && y != -1) {
				...
				if (name == 0) {
					...
					d = "Time: ";
					char p = (D1[x][y] - 1) / 10 + '0';
					d = d + p;
					p = (D1[x][y] - 1) % 10 + '0';
					d = d + p;
				}
				dis.SetText(d);
				dis.SetColor(TextObject::WHITE_TEXT);
